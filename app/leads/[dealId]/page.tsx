  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: dealRow, error } = await supabase
        .from("deals")
        .select(
          "id, contact_id, owner_id, title, stage, destination, value_amount, currency, probability, next_step, expected_close_at, source, created_at, last_activity_at",
        )
        .eq("id", params.dealId)
