...
type Payment = {
  ...
  status:
    "pending" | "partially_paid" | "paid" | "overdue" | "refunded" | "void";
  title: string;
...
export default function FinancePage() {
  ...
  const surface = pathname.startsWith("/suppliers") ? "suppliers" : "finance";
  ...
  useEffect(() => {
    if (pathname !== "/finance" || window.location.hash !== "#suppliers") return;
    ...
    void load().catch((error) => {
      setLoadError(
        error instanceof Error
          ? error.message
          : "AIOS could not load supplier and finance operations.",
      );
      setLoading(false);
    });
  }, [reloadKey]);
  ...
  const overduePayments = useMemo(
    () =>
      payments
        ...
        .sort((left, right) =>
          (left.due_at ?? "9999-12-31").localeCompare(
  ...
  const openReceivables = useMemo(
    () => openPayments.filter((payment) => payment.direction === "receivable"),
    [openPayments],
  );
  const openPayables = useMemo(
    () => openPayments.filter((payment) => payment.direction === "payable"),
    [openPayments],
  );
  const pendingFinanceApprovals =
    invoiceApprovals.filter((approval) => approval.status === "pending")
      .length +
    paymentLinkApprovals.filter((approval) => approval.status === "pending")
      .length;
  const priorityPayments = (
    overduePayments.length > 0 ? overduePayments : openPayments
  ).slice(0, 3);
  const financeNext = loading
    ? {
        title: "Loading the finance desk",
        detail: "Checking balances, due dates, and approval requests.",
        href: "#payment-ledger-title",
        action: "View ledger",
      }
    : overduePayments.length > 0
      ? {
          title: `Review ${overduePayments.length} overdue ${overduePayments.length === 1 ? "balance" : "balances"}`,
          detail:
            "Overdue customer and supplier obligations need a human decision before AIOS prepares any external follow-up.",
          href: "#payment-ledger-title",
          action: "Review overdue",
        }
      : pendingFinanceApprovals > 0
        ? {
            title: `Decide ${pendingFinanceApprovals} finance ${pendingFinanceApprovals === 1 ? "approval" : "approvals"}`,
            detail:
              "Invoice issuance and payment links stay paused until an authorized teammate reviews the exact evidence.",
            href: "/aios/approvals",
            action: "Review approvals",
          }
        : openReceivables.length > 0
          ? {
              title: `Track ${openReceivables.length} open ${openReceivables.length === 1 ? "receivable" : "receivables"}`,
              detail:
                "Customer balances are open. AIOS can prepare the next step, while sending and money movement remain approval-gated.",
              href: "#payment-ledger-title",
              action: "Open receivables",
            }
          : openPayables.length > 0
            ? {
                title: `Review ${openPayables.length} supplier ${openPayables.length === 1 ? "payable" : "payables"}`,
                detail:
                  "Supplier obligations are recorded and ready for an operations or finance review.",
                href: "#payment-ledger-title",
                action: "Open payables",
              }
            : {
                title: "Finance desk is clear",
                detail:
                  "There are no open balances or finance approvals requiring attention.",
                href: "#payment-ledger-title",
                action: "View ledger",
              };
  ...
  function downloadAccountingLedger() {
    if (!canManageFinance) {
      setNotice(
        "Only an owner, admin, or finance member can export the ledger.",
      );
      return;
    }
    if (loading) {
    ...
    startTransition(async () => {
      try {
        await createPaymentObligation({
          organizationId,
          direction:
            formData.get("direction") === "payable" ? "payable" : "receivable",
          title: String(formData.get("title") || ""),
  ...
  function submitVoid(event: FormEvent<HTMLFormElement>, payment: Payment) {
    event.preventDefault();
    if (!organizationId || pending) return;
    const form = event.currentTarget;
    const formData = new FormData(form);
  ...
  return (
    ...
    >
      <FeatureHeader
        ...
        ariaLabel={`${surface === "suppliers" ? "Supplier" : "Finance"} workspace navigation`}
      />

      <OperationalPageHeader
        section="Operations"
        title={surface === "suppliers" ? "Suppliers" : "Finance"}
        meta={
          surface === "suppliers"
            ? `${suppliers.length} supplier profiles`
            : `${openPayments.length} open items · ${overduePayments.length} overdue`
        }
      />

      <nav
        className="crm-record-tabs finance-record-tabs"
        aria-label={`${surface === "suppliers" ? "Supplier" : "Finance"} sections`}
      >
        {surface === "suppliers" ? (
          <>
            <a href="#suppliers">Directory</a>
            <a href="#supplier-rates">Products &amp; rates</a>
            <Link href="/finance">Open Finance</Link>
          </>
        ) : (
          <>
            <a href="#finance-overview">Overview</a>
            <a href="#payment-ledger-title">Receivables &amp; payables</a>
            <a href="#invoice-readiness-title">Invoicing</a>
            <a href="#accounting-export-title">Accounting export</a>
            <Link href="/suppliers">Open Suppliers</Link>
          </>
        )}
      </nav>

      <p className="crm-inline-boundary">
        {surface === "suppliers"
          ? "Supplier records and rates are internal. Contacting or committing to a supplier requires approval."
          : "Payments, refunds and invoice delivery require approval."}
      </p>
      ...
      {loadError ? (
        <ErrorState
          title={`${surface === "suppliers" ? "Suppliers" : "Finance"} is unavailable`}
        ...
        <>
          <section
            className="finance-priority"
            id="finance-overview"
            aria-labelledby="finance-priority-title"
          >
            <div className="finance-priority-copy">
              <p>AIOS FINANCE NEXT</p>
              <h2 id="finance-priority-title">{financeNext.title}</h2>
              <span>{financeNext.detail}</span>
              <Link href={financeNext.href}>{financeNext.action}</Link>
            </div>
            <div
              className="finance-priority-items"
              aria-label="Balances needing attention"
            >
              {priorityPayments.length === 0 ? (
                <p className="finance-priority-empty">
                  New receivables, payables, or approval requests will appear
                  here.
                </p>
              ) : (
                  ...
                  return (
                    <article key={payment.id}>
                      <div>
                        ...
                        <b>{payment.title}</b>
                        <small>Due {shortDate(payment.due_at)}</small>
                      </div>
                      <strong>{money(outstanding, payment.currency)}</strong>
                    </article>
                  );
                })
              )}
          ...
          <section className="finance-pulse" aria-label="Open finance summary">
            <article>
              ...
              <b>{openReceivables.length}</b>
              <small>customer balances open</small>
            </article>
            <article>
              <span>PAYABLES</span>
              <b>{openPayables.length}</b>
              <small>supplier balances open</small>
            </article>
            <article>
              <span>OVERDUE</span>
              <b>{overduePayments.length}</b>
              <small>need finance attention</small>
            </article>
            <article>
              <span>APPROVALS</span>
              <b>{pendingFinanceApprovals}</b>
              <small>issuance or payment links</small>
            </article>
          </section>

          <section
            className="currency-strip"
            aria-label="Outstanding by currency"
          >
            <header>
              <div>
                <p>OUTSTANDING BY CURRENCY</p>
                <h2>Money in, money out, and overdue</h2>
              </div>
            </header>
              ...
              <div>
                {currencySummaries.map(([currency, summary]) => (
                  <article key={currency}>
                    <b>{currency}</b>
                    <span>In {money(summary.receivable, currency)}</span>
                    <span>Out {money(summary.payable, currency)}</span>
                    <small>
                      {summary.overdue > 0
          ...
          <section className="finance-layout">
            <div className="supplier-workbench" id="suppliers">
                  ...
                  {selectedSupplier ? (
                    <article className="supplier-profile">
                      <header>
                        <div>
                          <p>
                            {selectedSupplier.category || "TRAVEL SUPPLIER"}
                          </p>
            ...
            <aside className="ledger-create">
              <div>
                ...
                <span>
                  Record what is due. Creating this item does not send an
                  invoice or move money.
                </span>
              </div>
              {canManageFinance ? (
                <details className="finance-action-drawer obligation-drawer">
                  <summary>
                    <span>Add obligation details</span>
                    <small>Receivable or payable</small>
                  </summary>
                ...
                <PermissionNotice
                  contained
                  description="You can inspect receivables and payables. Financial changes are limited to owners, administrators, and finance teammates."
                />
              )}
            </aside>
          </section>
            ...
            >
              <header className="finance-section-heading">
                <div>
                  ...
                  <span>
                    Freeze customer-safe line and payment evidence before a
                    legal number, issue date, or delivery channel is approved.
                  </span>
                </div>
                <span>
                  {
                    invoiceDrafts.filter((draft) => draft.status === "ready")
                      .length
                  }{" "}
                  ready
                </span>
              ...
              <div className="invoice-readiness-grid">
                <div className="invoice-control-stack">
                  <details className="finance-action-drawer invoice-control-drawer">
                    <summary>
                      <span>
                    ...
                    <form
                      className="invoice-policy-form"
                      onSubmit={submitInvoicePolicy}
                    >
                      <label>
                      ...
                      <small>
                        Saving this policy does not reserve or allocate an
                        invoice number. Issuance will require a separate
                      ...
                      <div>
                        <label>
                          ...
                          <input
                            ...
                            defaultValue={
                              invoiceIssuerProfile?.jurisdiction_country_code ??
                              "IN"
                            }
                            pattern="[A-Za-z]{2}"
                            maxLength={2}
                    ...
                    acceptedQuoteReceivableGroups.map((group) => {
                      ...
                      const policyCurrent =
                        draft?.status === "ready" && invoicePolicy
                          ? draft.number_policy_updated_at ===
                            invoicePolicy.updated_at
                          : false;
                      return (
                        <article
                        ...
                        >
                          <header>
                            <div>
                              <small>
                                ACCEPTED QUOTE · {group.count} MILESTONES
                              </small>
                              <strong>
                            ...
                            <>
                              <div className="invoice-draft-facts">
                                ...
                                <span>
                                  ...
                                  <b>
                                    {draft.line_count} lines ·{" "}
                                    {draft.payment_term_count} terms
                                  </b>
                                </span>
                              </div>
                              <p
                              ...
                              {issuance ? (
                                <div className="invoice-issued-evidence">
                                  ...
                                  <span>
                                    {money(
                                      issuance.total_amount,
                                      issuance.currency,
                                    )}{" "}
                                    · evidence{" "}
                                    {issuance.issuance_sha256.slice(0, 12)}…
                                  </span>
                                  <small>
                                    Permanent number allocated. Customer
                                    delivery, messaging, payment links and
                                    collection remain separate controlled
                                    workflows.
                                  </small>
                                  {invoiceDocument ? (
                                    <div className="invoice-document-evidence">
                                      <div>
                                        ...
                                        <small>
                                          ...
                                          {Math.ceil(
                                            invoiceDocument.byte_size / 1024,
                                          )}{" "}
                                          KB · checksum{" "}
                                          {invoiceDocument.content_sha256.slice(
                                            0,
                                            12,
                                          )}
                                          …
                                      ...
                                      <p>
                                        Jurisdiction review required before
                                        external delivery.
                                      </p>
                                    </div>
                                  ) : (
                                    <div className="invoice-document-readiness">
                                      <div>
                                        ...
                                        <strong>
                                          Create a private operational invoice
                                          record
                                        </strong>
                                        <small>
                                          The PDF will be checksum-bound to this
                                          exact issuance and stored privately.
                                        </small>
                                      </div>
                                      <button
                                        type="button"
                                      ...
                                      <p>
                                        Internal artifact only · jurisdiction
                                        review required · no customer delivery.
                                      </p>
                                    </div>
                                  )}
                                </div>
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    prepareInvoiceDraft(group.quoteId)
                                  }
                                  disabled={pending}
                                >
                                ...
                                <div className="invoice-approval-state approved">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      issueInvoice(draft.id, approval.id)
                                    }
                                    disabled={pending}
                                  >
                                  ...
                                  <small>
                                    This irreversible step consumes{" "}
                                    {draft.number_preview}
                                    atomically; it still performs no delivery or
                                    charge.
                                  </small>
                                </div>
                                ...
                                <div className="invoice-approval-state pending">
                                  <span>
                                    Waiting for a human issuance decision.
                                  </span>
                                ...
                                <p className="draft-stale">
                                  Save the legal issuer identity before
                                  requesting issuance approval.
                                </p>
                              )}
                            </>
                          ) : (
          ...
          {canManageFinance ? (
            ...
            >
              <div>
                ...
                <h2 id="accounting-export-title">
                  Export facts, not a second ledger.
                </h2>
                <span>
                  Download every visible obligation and immutable settlement as
                  separate formula-safe CSV rows. Currencies are never combined,
                  and exact quote and issuance references stay attached.
                </span>
              </div>
              <dl>
                <div>
                ...
                <div>
                  <dt>Currencies</dt>
                  <dd>
                    {new Set(payments.map((payment) => payment.currency)).size}
                  </dd>
                </div>
              </dl>
              <button
                type="button"
                onClick={downloadAccountingLedger}
                disabled={loading}
              >
                Download accounting CSV
              </button>
              <small>
                Finance role only · includes internal identifiers and free-text
                ledger details · filters do not narrow the export · no
                accounting upload or external effect
              </small>
            </section>
          ) : null}
          ...
          <section
            ...
            id="ledger"
            aria-labelledby="payment-ledger-title"
          >
                ...
                {filteredPayments.map((payment) => {
                  ...
                  const paymentLinkApproval = paymentLinkDraft
                    ? paymentLinkApprovals.find(
                        (candidate) =>
                          candidate.entity_id === paymentLinkDraft.id,
                      )
                    : null;
                    ...
                    >
                      ...
                      {payment.quote_acceptance_id ? (
                        <div className="quote-receivable-provenance">
                          ...
                          <small>
                            Internal ledger record only · no invoice document
                      ...
                      {canPrepareCollection ? (
                        ...
                        >
                          ...
                          {!paymentLinkDraft ? (
                            <>
                              <p>
                                Freeze this issued receivable&apos;s current
                                full outstanding balance before asking a human
                                to approve provider handoff.
                              </p>
                              <button
                                type="button"
                                onClick={() =>
                                  prepareCollectionRequest(payment.id)
                                }
                                disabled={pending}
                              >
                            ...
                            <div className="payment-link-sandbox-active">
                              ...
                              <small>
                                {paymentLinkExecution.adapter_version} ·
                                reference{" "}
                                {paymentLinkExecution.provider_reference} ·
                                expires{" "}
                                {new Date(
                                  paymentLinkExecution.checkout_expires_at,
                                ).toLocaleString()}
                            ...
                            <div className="payment-link-approved">
                              ...
                              <p>
                                Execute the local adapter to test idempotency
                                and the checkout contract. It cannot contact a
                                payment provider or collect money.
                              </p>
                              <button
                                type="button"
                                onClick={() =>
                            ...
                            <div className="payment-link-expired">
                              ...
                              <p>
                                The previous simulation expired or its
                                receivable evidence changed. Re-route the
                                current exact draft.
                              </p>
                              <form
