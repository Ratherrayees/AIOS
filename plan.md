...
| CRM | Release-candidate core | Contacts, atomic import, search, tenant-enforced ownership, private saved views across Contacts, Leads, Tasks, Inbox, and Analytics, companies, consent evidence/preferences, human-reviewed duplicate merging, public attributed lead forms, reusable qualification contracts, evidence-gated pipeline transitions and history, internal follow-up playbooks, response/follow-up SLAs, tiered agentic escalation, source/velocity analytics, tasks, secure quote drafts, activity history, explicit role policies, and audited mutations | Richer traveller profiles, governed drag/drop, and product-owner acceptance |
...
| Database security | Foundation delivered | RLS on all 46 application tables, explicit Data API grants, reviewed role/action policies, immutable tenant identity, same-tenant foreign keys, opt-in MFA enforcement, private role-scoped document storage, atomic document/timeline/audit recording, guarded sales-workflow RPCs, complete generated live-schema types, deterministic fictional seed data, clean zero-state migration replay, zero schema-lint/advisor findings, and live anonymous/authenticated authorization probes | Production backup/restore drill and workload-informed index review |
| Release engineering | Working foundation | Initialized Git repository and reviewed baseline commit, lockfile, exact dependency and CLI versions, CI, source secret scanning, strict lint/typecheck, 105 behavioral tests, 15 zero-provider AI safety evaluations, 23 Chromium journeys including authenticated tenant switching, public capture, governed pipeline movement, qualification gates, internal playbooks, approval resolution, analytics, private upload, and mobile coverage, production build, zero-vulnerability audit, migration replay/type-drift gate, Supabase probes, threat model, credential-rotation guidance, and database-recovery procedure | Remote/branch protection, preview/staging/prod separation, observability, cross-browser/assistive-tech evidence, a completed restore drill, and deployment runbook |
...
| 05 Database and RLS | 95% | 15 Supplier and finance | 12% |
| 06 Security and privacy | 74% | 16 Operations automation | 20% |
| 07 CRM core | 94% | 17 Knowledge/RAG | 8% |
| 08 Communication hub | 70% | 18 Intelligence/reporting | 30% |
| 09 Lead pipeline | 98% | 19 QA/security review | 56% |
...
- **Done:** The Supabase access verifier confirms administrative visibility and no anonymous row exposure across all 46 application tables and denies anonymous public-lead, document, qualification, and follow-up RPC execution. A separate authenticated probe creates isolated owner/viewer tenants and proves tenant isolation, viewer read-only behavior, role-escalation denial, owner-invitation denial, same-tenant relationship enforcement—including template/draft, AIOS-budget, model-price, and AI-job/run links—private saved-view isolation, immutable tenant identity and price versions, governed deal-stage history, evidence-gated stage advancement, atomic owner-assigned follow-up tasks, server-only public capture, server-only job claim/settlement/requeue, lease ownership, retry transitions, and private document-storage boundaries; it also rejects forged document metadata and direct qualification-evidence mutation before removing its fixtures.
...
**Current progress:** The workspace is a valid Git repository on `main` with reviewed checkpoint commits authored as Rayees Amin; generated artifacts, local Supabase state, Playwright reports, and deployment caches are ignored. Node 24, Next.js 16.2.12, TypeScript 5.9.3, React 19.2.8, Supabase JS 2.110.9, Supabase CLI 2.110.0, and all direct dependencies are pinned exactly in the lockfile. CI runs secret scanning, a zero-state Supabase migration/seed replay, schema lint, generated-type drift detection, 105 behavioral tests, 15 zero-provider AI evaluations, strict typecheck, ESLint, a production build, 23 Chromium journeys, conditional live Supabase authorization probes, and a high-severity dependency audit. Remaining work is a remote with branch protection, minimum package-age enforcement, deployment/rollback automation, and separate preview, staging, and production environments.
...
**Current progress:** Local release verification currently passes a clean migration replay, generated-type drift check, schema lint, Supabase security and performance advisors, 105 behavioral tests, 15 no-provider AI safety evaluations, strict TypeScript, ESLint, source secret scanning, a production build, zero known npm vulnerabilities, a 46-table anonymous-access probe, and a disposable owner/viewer authorization suite covering cross-tenant reads/writes/RPCs, governed stage changes, qualification evidence, internal follow-up playbooks, lead capture, and private storage. The same access and authorization probes pass against the hosted project, including explicit browser mutation denial on the six sales-workflow tables, and the hosted migration ledger is synchronized. All 23 Chromium journeys pass, including public lead capture, authenticated tenant switching, protected modules, mobile overflow, response acknowledgement, commercial planning, atomic evidence-gated stage movement, internal sequence tasks, analytics, human approval resolution, and private document upload. Remaining work is CI confirmation on a protected remote, WebKit/Firefox, authenticated keyboard/assistive-technology testing, load/failure/webhook tests, production-like restore and rollback drills, external review, and launch-threshold acceptance.
