...
| Database security | Foundation delivered | RLS on all 58 application tables, explicit Data API grants, reviewed role/action policies, immutable tenant identity, same-tenant foreign keys, opt-in MFA enforcement, private role-scoped document storage, atomic document/timeline/audit recording, guarded sales, trip-lifecycle, traveler-entry, knowledge renewal, operational-exception/schedule, payment-ledger, and traveler-portal RPCs, complete generated live-schema types, deterministic fictional seed data, clean zero-state migration replay, zero schema-lint findings, and live anonymous/authenticated authorization probes | Production backup/restore drill and workload-informed index review |
| Release engineering | Working foundation | Initialized Git repository and reviewed baseline commit, lockfile, exact dependency and CLI versions, CI, source secret scanning, strict lint/typecheck, 142 behavioral tests, 21 zero-provider AI safety evaluations, and 44 Chromium journeys covering the complete implemented UI surface, authenticated tenant switching, public capture, governed drag/drop, qualification gates, internal playbooks, operational trip handoff, traveler entry readiness, Suppliers & Finance, durable Radar schedules, governed knowledge renewal and grounded refusal, approval resolution, traveler-portal publication/download/revocation, analytics, private upload/download, MFA, mobile coverage, and a zero-console-problem route sweep; production build, zero-vulnerability audit, migration replay/type-drift gate, Supabase probes, threat model, credential-rotation guidance, and database-recovery procedure | Remote/branch protection, preview/staging/prod separation, observability, cross-browser/assistive-tech evidence, a completed restore drill, and deployment runbook |

### Completion scorecard

These percentages estimate implemented and verified product scope against each phase's exit gate; they are not claims that a phase is formally accepted. Overall implementation is approximately **73%**. Under the strict cross-phase rule requiring product-owner acceptance, no phase is formally closed yet.
...
| 07 CRM core | 98% | 17 Knowledge/RAG | 72% |
| 08 Communication hub | 72% | 18 Intelligence/reporting | 30% |
| 09 Lead pipeline | 99% | 19 QA/security review | 85% |
...
**Current verification checkpoint (29 July 2026):** 142 behavioral tests, 21 AI safety evaluations, 44 Chromium journeys, 58 protected tables, 32 guarded RPC checks, and 208 authenticated authorization assertions pass. Lower counts in the chronological bullets below describe the checkpoint at which that capability first landed.
...
**Current progress:** The workspace is a valid Git repository on `main` with reviewed checkpoint commits authored as Rayees Amin; generated artifacts, local Supabase state, Playwright reports, and deployment caches are ignored. Node 24, Next.js 16.2.12, TypeScript 5.9.3, React 19.2.8, Supabase JS 2.110.9, Supabase CLI 2.110.0, and all direct dependencies are pinned exactly in the lockfile. CI runs secret scanning, a zero-state Supabase migration/seed replay, schema lint, generated-type drift detection, 142 behavioral tests, 21 zero-provider AI evaluations, strict typecheck, ESLint, a production build, 44 Chromium journeys, conditional live Supabase authorization probes, and a high-severity dependency audit. Remaining work is a remote with branch protection, minimum package-age enforcement, deployment/rollback automation, and separate preview, staging, and production environments.
...
**Current progress:** All 58 application tables have RLS and explicit grants. Core contacts, companies, deals, stage/qualification/trip/exception history, lead forms/submissions, reusable qualification and follow-up definitions, tasks, activity, inbox, message templates/drafts, AIOS budget/job/model-price policy, governed knowledge sources/sections, suppliers and their contacts/contracts, travelers, human-reviewed entry checks, bookings, governed payment obligations/allocations, documents, traveler-portal links/document mappings, operational exceptions, and itinerary comments use reviewed role/action policies; viewers are read-only, authenticated messages are internal-only, and ordinary users have no destructive core-record policies. A generic trigger makes `organization_id` immutable on mutable tenant tables. Composite foreign keys prevent CRM, inbox, lead-capture, sales-workflow, draft, AI lineage, AI job/run, model price/budget, knowledge, quote, trip, traveler/readiness, itinerary, booking, supplier, payment, allocation, document, portal, and operational-exception rows from referencing another organization. The private `travel-documents` bucket requires a tenant/document UUID path, restricts content type and size, enforces role and opt-in MFA checks, and offers no browser overwrite/delete policy. Its guarded recorders verify that the exact user-owned storage object exists, then atomically create the appropriate lead- or trip-linked document record, activity event, and audit event; failed recording removes the exact orphaned object server-side. Deal stages, trip statuses, traveler-entry evidence, knowledge review state, exception lifecycle state, payment-obligation state, portal publication/revocation state, and immutable settlement evidence are protected from direct browser mutation and move through row-locked, role-checked functions with actor evidence. Won-deal conversion is row-locked and idempotent, creates one operational trip and lead traveller, and preserves actor/audit evidence. Required qualification evidence is also read-only to browsers and changes only through actor-recording RPCs. Follow-up templates instantiate bounded internal tasks atomically and cannot be applied twice to one opportunity. The application consumes complete CLI-generated live-schema TypeScript definitions, with a narrow override only where the generator cannot express nullable RPC arguments. A deterministic fictional seed and clean local reset replay every migration from zero. Schema lint returns no findings. Local authorization probes prove two-tenant read/write/RPC isolation, viewer denial, governed deal/trip/readiness/knowledge/exception/payment/portal transitions and history, qualification gates, playbook deduplication, private capture and document boundaries, safe public snapshots, approval-bound publishing, token rotation/revocation, and forged-document rejection. The Phase 14 through Phase 17 migrations still need a staged hosted deployment before the hosted probe can cover the new records. Remaining work is that deployment, a current security/performance-advisor review, an isolated production-like backup/restore drill, and workload-based review of informational unused-index findings.
...
**Current progress:** `/knowledge` now provides a tenant-scoped curator workbench, stale-evidence queue, approved-retrieval preview, and a grounded Answer Desk. Owners, admins, trip designers, and operations users can create versioned drafts for destination guides, visa advisories, supplier terms, SOPs, policies, product sheets, and other evidence; classify source authority and normal/restricted sensitivity; record an HTTPS source link, validity window, summary, and review deadline; and add ordered citation-ready passages. Direct browser writes are denied. Guarded RPCs enforce the legal `draft → in review → approved → retired` lifecycle, freeze metadata and passages while a source is in review, require at least one passage and a current review deadline before approval, separate curator access from approval authority, preserve reviewer/audit evidence, and prevent approved content from being edited in place. A curator can prepare a successor draft from an approved source, clone its citations, revise or remove passages only while the successor remains Draft, and send the replacement through human review; approving it atomically retires the superseded version, preserves explicit lineage, and avoids a retrieval gap. Competing active successors are rejected. Ordinary members can read and search only approved normal-sensitivity material; restricted content remains curator-only, drafts never enter retrieval, retired sources disappear immediately, and expired review dates are returned explicitly as stale. Full-text retrieval is tenant- and permission-aware and returns the exact source, version, authority, link, citation label, excerpt, relevance, and freshness state. The Answer Desk sends only current permitted evidence through the governed provider router, requires structured claims tied to exact retrieved passage IDs, validates lexical and date/number support, attaches authoritative citations server-side, records a durable job/run with token-cost telemetry, and escalates high-impact visa/passport/immigration/legal/medical/refund/payment/price/booking topics to a human. Unsupported or stale-only questions receive an explicit deterministic refusal without a provider call. Question and source text are treated as untrusted input, prompt-like content is blocked, and direct identifiers are redacted before model use. Local verification covers lifecycle and renewal boundaries, permission-aware retrieval, grounding/citation attacks, invented facts, model-input safety, high-impact escalation, zero-provider refusal, durable metadata-only jobs, and the complete browser workflow. A fictional non-CRM GLM-4.7-Flash smoke test confirms the provider adapter accepts the structured answer contract. Remaining scope is private file ingestion/parsing, automated chunking, richer destination/supplier import tools, pgvector-backed semantic retrieval where lexical retrieval is insufficient, source-conflict resolution, bulk freshness policy, staged deployment, and product-owner acceptance.
...
**Current progress:** Local release verification currently passes a clean migration replay, generated-type drift check, schema lint, 142 behavioral tests, 21 no-provider AI safety evaluations, strict TypeScript, ESLint, source secret scanning, a production build, zero known npm vulnerabilities, a 58-table anonymous-access probe with 32 guarded RPC checks, and 208 disposable owner/viewer authorization assertions covering cross-tenant reads/writes/RPCs, governed deal/trip/traveler-entry/knowledge/exception/schedule/payment/portal changes, idempotent won-deal conversion, qualification evidence, internal follow-up playbooks, lead capture, lease-owned Operations Radar scheduling, supplier finance, permission-aware cited retrieval, source renewal, grounded Answer Desk output, approval-bound traveler publishing, token rotation, immediate revocation, and private storage. The prior hosted schema and probes were synchronized; the operational-trip, finance, Operations Radar, durable-schedule, traveler-entry, traveler-portal, and knowledge migrations are intentionally pending a staging-first hosted deployment. All 44 Chromium journeys pass, including public lead capture, authenticated tenant switching, dynamic signed-in dashboard identity, every implemented workspace surface, mobile overflow, response acknowledgement, commercial planning, atomic evidence-gated stage movement, governed drag/drop plus selector fallback, Contacts/Inbox/Tasks/Quotes/Itinerary/Trip Operations/Suppliers & Finance/Operations Radar/Traveler Portal/Knowledge/Team/AIOS mutation workflows, human-reviewed passport/visa readiness, durable schedule configuration/execution/history, internal sequence tasks, permission-aware knowledge approval, renewal, draft revision, atomic supersession, cited retrieval, deterministic unsupported refusal without model execution, analytics, human approval resolution, safe public snapshots, approved voucher download, immediate portal revocation, private document upload/download, live TOTP enrollment/removal, and a protected-route sweep with no browser errors or warnings. The complete matrix and defects corrected during the review are recorded in `docs/ui-wiring-audit.md`. Remaining work is a current security/performance-advisor review, CI confirmation on a protected remote, staged migration deployment, WebKit/Firefox, authenticated keyboard/assistive-technology testing, load/failure/webhook tests, production-like restore and rollback drills, external review, and launch-threshold acceptance.
