...
- Authentication sessions, MFA factors, invitation tokens, and role assignments
...
| --- | --- | --- |
| Cross-tenant read/write | RLS on 31 application tables, active-membership helpers, immutable `organization_id`, same-tenant composite foreign keys, live owner/viewer negative probe | Extend authenticated browser and RPC coverage |
...
| Forged or replayed approval | Pending-only insert policy, no direct client update/delete, row lock, single transition, atomic audit event | Add durable post-approval job execution/retry state |
...
| Private document disclosure | Private bucket, tenant-first UUID paths, role/MFA policies, restricted MIME/size, no client overwrite/delete | Add signed-URL service and access audit before UI release |
