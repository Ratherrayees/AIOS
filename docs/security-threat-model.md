...
- Authentication sessions, MFA factors, invitation tokens, public proposal/traveler/sandbox-payment bearer tokens, and role assignments
...
8. Approved payment execution to a provider adapter: the adapter receives only exact internal evidence plus a deterministic idempotency key; sandbox output remains local, while any future live adapter becomes an external-effect boundary requiring separate secrets, network, webhook, and reconciliation controls.

## Primary threats and controls

| Threat | Existing controls | Residual work |
| --- | --- | --- |
| Cross-tenant read/write | RLS on 80 application tables, active-membership helpers, immutable `organization_id`, same-tenant composite foreign keys, 439 owner/viewer/operations/service authorization assertions | Repeat the complete probe against staging and production-like identities |
...
| Premature, forged, duplicated, or silently rewritten invoice | Finance-only draft/issuer/issuance visibility; owner/admin/finance plus MFA; exact accepted quote/version/acceptance/schedule/receivable/contact relationships; line/term/net/tax/total reconciliation; role-protected issuer and tax identity; canonical draft/issuer hashes in an expiring human gate; automatic stale-approval expiry; current-approver recheck; row locks; atomic sequence consumption, draft transition and receivable linkage; immutable issuance hash; idempotent retry; prefix-sequence and invoice-number uniqueness; sequence rewind denial; browser-write denial; content-safe audit; and 439 tenant/role/service assertions | Approve jurisdiction-specific tax/legal wording before external use, then add delivery retry/idempotency evidence and void/credit-note rules; permanent issuance remains distinct from rendering and delivery |
| Forged, replaced, leaked, or misrepresented invoice PDF | Explicit finance-only render action; deterministic versioned renderer; immutable issuance source hash plus PDF byte SHA-256; dedicated non-public 2 MiB PDF-only bucket; service-role-only upload and evidence registration; exact tenant/issuance/version/hash object path; active finance-actor attribution; browser upload/insert/update/delete denial; finance/MFA RLS for metadata and object reads; 60-second signed downloads; access audit; multi-page visual QA; and a permanent `jurisdiction_review_required` label | Approve a jurisdiction-specific template before delivery, back up and restore Storage bytes in staging, define renderer-version retirement rules, and add approval-gated customer delivery without exposing signed URLs in durable payloads |
| Premature, stale, or unauthorized payment-link creation | Finance-only immutable request drafts; owner/admin/finance plus MFA; exact open-receivable, permanent-issuance, currency, due-date, updated-time and outstanding-balance evidence; SHA-256 binding; full-balance-only v1 scope; canonical expiring human approval; Operations resolution denial; direct-write denial; row locks and idempotent retries; automatic draft supersession and pending/approved gate expiry after payment mutation; explicit zero-provider-link/message/charge/settlement evidence before execution | Select a production payment provider and approve its region/data/fee/idempotency policy; the existing provider contract must remain exact-evidence and approval bound |
| Forged, replayed, leaked, or misrepresented provider execution | Provider-neutral typed request/result contract; deterministic PII-free idempotency key independently recomputed in PostgreSQL; service-role-only recorder and public-snapshot RPCs; current finance-actor recheck; exact approved/unexpired draft/payment/issuance/hash revalidation under row locks; immutable provider/environment/adapter/reference/token-hash/expiry evidence; one active execution per draft; exact retry; replacement and payment-change invalidation; finance-only RLS; 256-bit sandbox token stored only as SHA-256; customer-safe active-snapshot projection; production-default sandbox disable; and explicit zero-network/zero-real-money audit flags | Choose and threat-model the live adapter, store its secrets only in deployment infrastructure, use provider-native idempotency, verify signed webhooks/replay/order/amount/currency, reconcile to immutable allocations, redact checkout URLs from logs, and define refund/chargeback handling before real money is enabled |
| Forged, replayed, mismatched, or over-authoritative provider event | Provider-neutral event contract; deterministic sandbox event ID and payload SHA-256 independently recomputed in PostgreSQL; service-only guarded recorder with direct service/browser mutation revoked; current finance-actor and exact execution/reference recheck; event-time bounds; provider/environment/event-ID deduplication; immutable finance-only source/integrity/amount/currency/status/reason evidence; explicit `matched_unposted` state; synthetic source and unsigned status labels; audit proof of zero webhook/network/payment/settlement effect; and separate human settlement evidence | Implement the selected provider's raw-body signature verification, timestamp/replay window, account-to-tenant routing, event ordering, mismatch queue, real reconciliation workflow, secret rotation, refund/dispute handling, and production observability before accepting external events |
| Unsafe or misleading accounting export | Owner/admin/finance-only UI, tenant RLS as source boundary, separate obligation and settlement records, stable quote/acceptance/issuance references, no cross-currency aggregation, formula neutralization, fail-closed amount/currency/linkage validation, explicit read-only metadata, and no upload/provider integration | Define encrypted storage, approved sharing/retention practice, field mapping and reconciliation sign-off for each chosen accounting platform before operational import |
